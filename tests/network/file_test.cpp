...
namespace {
    ...
    // delete file
    void prepareEnv() { std::system("[[ -f fileTest.txt ]] && rm fileTest.txt"); }
    ...
    std::vector<CryptoPacket> packets{
        {Header::Type::FileName, 7, "fileTest.txt"},
        {Header::Type::FileData, 12, "Hello World!"},
        {Header::Type::Hash, 64,
         "7f83b1657ff1fc53b92dc18148a1d65dfc2d4b1fa3d677284addd200126d9069"},
        {Header::Type::Exit, 0, ""}};

    std::vector<CryptoPacket> withMissedPackets0{
        {Header::Type::FileData, 12, "Hello World!"},
        {Header::Type::Hash, 64,
         "7f83b1657ff1fc53b92dc18148a1d65dfc2d4b1fa3d677284addd200126d9069"},
        {Header::Type::Exit, 0, ""}};

    std::vector<CryptoPacket> withMissedPackets1{
        {Header::Type::FileName, 7, "fileTest.txt"},
        {Header::Type::Hash, 64,
         "7f83b1657ff1fc53b92dc18148a1d65dfc2d4b1fa3d677284addd200126d9069"},
        {Header::Type::Exit, 0, ""}};
    ...
    TEST(FileWriterTest, test_cryptoProcess) {
        ...
        for (const auto curPack : packets) {
            ...
            if (curPack.header.type == Header::Type::FileName) {
                auto expectedFile(
                    string(curPack.data.begin(), curPack.data.begin() + curPack.header.length));
        ...
        auto fileName(
            string(packets[0].data.begin(), packets[0].data.begin() + packets[0].header.length));
        auto fileData = readFileIntoString(fileName);
        auto expectedData(
            string(packets[1].data.begin(), packets[1].data.begin() + packets[1].header.length));
    ...
    TEST(FileReaderTest, test_ctr) {
        auto tsQueues = get2Queue();
        EXPECT_NO_THROW(
            FileReader<CryptoPacket> fileReader("fileReadTest.txt", tsQueues[0], tsQueues[1]));
    }
    ...
    void generateRandomFile(const std::string& filename, size_t sizeInBytes) {
        ...
        if (outFile.good()) {
            std::cout << "File '" << filename << "' created with " << sizeInBytes
    ...
    TEST(FileReaderTest, test_process) {
        ...
        threads.emplace_back([&]() {
            FileReaderMock<CryptoPacket> fileReader(filename, tsQueues[0], tsQueues[1]);
