...
namespace network {
    ...
    public:
        FileReader(const std::string& name, std::shared_ptr<ThreadSafeQueue<DataType>> curQueue,
    ...
    FileReader<DataType>::FileReader(const std::string& name,
                                     ...
                                     std::shared_ptr<ThreadSafeQueue<DataType>> nextQueue)
        : DataProcessor<FileReader<DataType>, DataType>(curQueue, nextQueue), fileName_(name) {
    ...
    void FileReader<DataType>::processDataImpl() {
        ...
        this->data_->header.length =
            std::fread(&this->data_->data, sizeof(this->data_->data[0]), PACKET_DATA_SIZE, file_);
