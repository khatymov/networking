...
namespace network {
    ...
    ClientHandler<T>::ClientHandler(const ConsoleParams& params)
        : endpoint_(tcp::endpoint(boost::asio::ip::make_address(params.ip.data()),
        ...
        tsQueues_ = {
            std::make_shared<ThreadSafeQueue<T>>(true)  // <- FileReader
            ,
        ...
        // reader is entry point. All works starts from reading from file and then send it to
        ...
        encryptor_ = std::make_unique<Encryptor<T>>(namedQueues[EncryptorKey].first,
                                                    namedQueues[EncryptorKey].second);
        ...
        connection_ =
            std::make_shared<Connection<T>>(std::move(socket_), namedQueues[ConnectionKey].first,
