...
namespace network {
    ...
    // waitNextData - wait available data for current queue (some other component should give it to
    // us) notifyComplete - give data that we got in processDataImpl to the next component
    ...
    class Connection : public std::enable_shared_from_this<Connection<DataType>>,
                       public DataProcessor<Connection<DataType>, DataType> {
    ...
    public:
        // for server
        explicit Connection(boost::asio::ip::tcp::socket socket, NamedQueue<DataType> namedQueues);

        // for client
        explicit Connection(boost::asio::ip::tcp::socket socket, QueuePtr<DataType> currentQueue,
        ...
        [[maybe_unused]] static bool read(boost::asio::ip::tcp::socket& socket,
                                          std::unique_ptr<DataType>& data,
                                          boost::system::error_code& ec);
        [[maybe_unused]] static bool write(boost::asio::ip::tcp::socket& socket,
                                           std::unique_ptr<DataType>& data,
                                           boost::system::error_code& ec);

        // Server functions
        // run async read/write via socket (recursive call of functions below: [readHead->
        // readPayload-> writeHead(ack)-> readHead->..]
        void run();
        void readHead();
        void readPayload();
        void writeHead();
        ...
        std::unique_ptr<DataType> ackPackPtr =
            std::make_unique<DataType>(DataType({Header::Type::Ack}));
        std::unique_ptr<DataType> nackPackPtr =
            std::make_unique<DataType>(DataType({Header::Type::Nack}));
    ...
    Connection<DataType>::Connection(boost::asio::ip::tcp::socket socket,
        ...
        : DataProcessor<Connection<DataType>, DataType>(namedQueues[ConnectionKey].first,
    ...
    void Connection<DataType>::readHead() {
        ...
        boost::asio::async_read(
            socket_, boost::asio::buffer(&this->data_->header, sizeof(this->data_->header)),
    ...
    void Connection<DataType>::readPayload() {
        auto self(this->shared_from_this());
        boost::asio::async_read(
            socket_, boost::asio::buffer(&this->data_->data, this->data_->header.length),
            [this, self](boost::system::error_code errorCode, std::size_t length) {
                if (!errorCode) {
                    if (this->data_->header.length > 0) {
                        writeHead();
    ...
    void Connection<DataType>::writeHead() {
        ...
        boost::asio::async_write(
            socket_, boost::asio::buffer(&ackPackPtr->header, sizeof(ackPackPtr->header)),
            [this, self](boost::system::error_code errorCode, std::size_t /*length*/) {
                if (!errorCode) {
                    this->notifyComplete();
                    if (not this->isDone()) {
    ...
    template <typename DataType>
    Connection<DataType>::Connection(boost::asio::ip::tcp::socket socket,
                                     QueuePtr<DataType> currentQueue, QueuePtr<DataType> nextQueue)
    ...
    template <typename DataType>
    bool Connection<DataType>::write(boost::asio::ip::tcp::socket& socket,
                                     std::unique_ptr<DataType>& packet,
                                     boost::system::error_code& ec) {
        // send header
        auto sentHeaderSize =
            boost::asio::write(socket, boost::asio::buffer(&packet->header, sizeof(packet->header)),
                               boost::asio::transfer_exactly(sizeof(packet->header)), ec);
        if (ec) {
        ...
        if (packet->header.length > 0) {
            auto sentPayloadSize =
                boost::asio::write(socket, boost::asio::buffer(packet->data, packet->header.length),
                                   boost::asio::transfer_exactly(packet->header.length), ec);
            if (ec) {
    ...
    template <typename DataType>
    bool Connection<DataType>::read(boost::asio::ip::tcp::socket& socket,
                                    std::unique_ptr<DataType>& packet,
                                    boost::system::error_code& ec) {
        auto headerSize =
            boost::asio::read(socket, boost::asio::buffer(&packet->header, sizeof(packet->header)),
                              boost::asio::transfer_exactly(sizeof(packet->header)), ec);
