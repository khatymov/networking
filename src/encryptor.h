...
namespace network {
    ...
    template <typename DataType>
    Encryptor<DataType>::Encryptor(std::shared_ptr<ThreadSafeQueue<DataType>> currentQueue,
                                   std::shared_ptr<ThreadSafeQueue<DataType>> nextQueue)
    ...
    void Encryptor<DataType>::processDataImpl() {
        ...
        try {
            ...
            ArraySink arraySink(&cipherData_->data[0], this->data_->header.length + AES::BLOCKSIZE);
